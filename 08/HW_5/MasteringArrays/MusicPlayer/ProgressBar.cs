...
namespace MusicPlayer
{
    public class ProgressBar : IDisposable, IProgress<double>
    {
        ...
        public ProgressBar()
        {
            timer = new Timer(TimerHandler, new AutoResetEvent(false),
                              TimeSpan.FromSeconds(1.0 / 8),
                              TimeSpan.FromSeconds(1.0 / 8));
        ...
        private void TimerHandler(object state)
        {
            lock (timer)
            {
                ...
                var text =
                    $"Play:  {new string('\u2588', progressBlockCount)}" +
                    $"{new string('_', blockCount - progressBlockCount)}- " +
